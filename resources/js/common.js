...
$(function()
{
	var emptyOutputMsg = "Java code will appear here";
	...
	$('#input').keyup(function()
	{
		...
		try {
			var output = curlToJava(input);
			if (output) {
				var coloredOutput = hljs.highlight("java", output);
	...
	$('#example4').click(function() {
		$('#input').val('curl -u "demo" -X POST -d @file1.txt https://example.com/upload').keyup();
	});
